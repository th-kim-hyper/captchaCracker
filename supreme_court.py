...
def init():
    ...
    if train_data.pred_data_list:
        pred_image = train_data.pred_data_list[0]
...
def gui():
    ...
    window.title("Win Captcha : 대법원")  # 제목 지정
    ...
    btnFileBrowser.grid(row=0, column=0, padx=4, pady=4)  # 버튼을 윈도우 창에 배치
    ...
    btnExec = Button(window, width=20, text='실행')  # 버튼 생성
    btnExec.grid(row=1, column=0, padx=4, pady=4, columnspan=3)  # 버튼을 윈도우 창에 배치
    ...
    canvas.grid(row=2, column=0, padx=4, columnspan=2)

    txtPred = Entry(window, width=8, font=('Arial 24'), justify='center')
    txtPred.grid(row=3, column=0, padx=4, columnspan=3)
